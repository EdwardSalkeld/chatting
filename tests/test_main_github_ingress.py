...
from app.main_github_ingress import _load_config, main
...
class MainGitHubIngressTests(unittest.TestCase):
    def test_load_config_rejects_unknown_keys(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            config_path = Path(tmpdir) / "github-ingress.json"
            config_path.write_text(json.dumps({"unexpected": True}), encoding="utf-8")

            with self.assertRaisesRegex(ValueError, "unknown keys"):
                _load_config(str(config_path))

    def test_main_publishes_assignment_event_once_and_uses_checkpoint(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch(
                    "app.main_github_ingress.BBMBQueueAdapter",
                    return_value=broker,
                ),
                patch(
                    "app.main_github_ingress.fetch_assignment_events_for_repository",
                    side_effect=[[event], [event]],
                ),
                patch(
                    "sys.argv",
                    [
                        "main_github_ingress.py",
                        "--db-path",
                        db_path,
                        "--bbmb-address",
                        "127.0.0.1:9876",
            ...
            self.assertEqual(broker.ensured, ["chatting.tasks.v1"])
